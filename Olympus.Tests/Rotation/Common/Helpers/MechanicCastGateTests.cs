...
public class MechanicCastGateTests
{
    ...
    [Fact]
    public void CastThrough_AlwaysReturnsFalse_EvenWhenRaidwideImminent()
    {
        var ctx = BuildContext(raidwide: new MechanicPrediction { SecondsUntil = 1f });
        Assert.False(MechanicCastGate.ShouldBlock(ctx.Object, castTime: 2.5f));
    }

    [Fact]
    public void CastThrough_AlwaysReturnsFalse_EvenWhenTankbusterImminent()
    {
        var ctx = BuildContext(tankbuster: new MechanicPrediction { SecondsUntil = 1.5f });
        Assert.False(MechanicCastGate.ShouldBlock(ctx.Object, castTime: 2.0f));
    }

    [Fact]
    public void CastThrough_IgnoresEnableToggle()
    {
        var ctx = BuildContext(enableGate: true, raidwide: new MechanicPrediction { SecondsUntil = 1f });
        Assert.False(MechanicCastGate.ShouldBlock(ctx.Object, castTime: 2.5f));
    }
