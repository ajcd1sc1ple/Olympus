...
public class BossModTimelineMergeTests
{
    ...
    [Fact]
    public void MergeRaidwide_PrefersBossModWhenSooner()
    {
        var cactbot = new MechanicPrediction(12f, TimelineEntryType.Raidwide, "Cactbot RW", 0.85f);
        var merged = BossModTimelineMerge.MergeRaidwide(4f, null, cactbot);

        Assert.NotNull(merged);
        Assert.Equal("BossMod raidwide", merged!.Value.Name);
        Assert.Equal(4f, merged.Value.SecondsUntil);
    ...
    [Fact]
    public void MergeTankBuster_UsesCastHintWhenSoonerThanStateMachine()
    {
