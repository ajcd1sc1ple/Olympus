...
public sealed class ProtractionHandler : IHealingHandler
{
    ...
    public void CollectCandidates(IAthenaContext context, RotationScheduler scheduler, bool isMoving)
    {
        ...
        if (hpPercent > config.ProtractionThreshold && !tankBusterImminent) return;
        ...
        scheduler.PushOgcd(AthenaAbilities.Protraction, target.GameObjectId, priority: protractionPriority,
            onDispatched: _ =>
            {
                ...
                context.Debug.PlanningState = capturedTb ? "TB Protraction" : "Protraction";

                if (context.TrainingService?.IsTrainingEnabled == true)
                {
                    var targetName = capturedTarget.Name?.TextValue ?? "Unknown";
                    var shortReason = capturedTb
                        ? $"Protraction on {targetName} before tankbuster"
                        : $"Protraction on {targetName} at {capturedHpPercent:P0}";
                    var factors = new[]
                    {
                        $"Target HP: {capturedHpPercent:P0}",
                        $"Threshold: {config.ProtractionThreshold:P0}",
                        capturedTb ? "Tank buster imminent — max HP prep" : "Increases max HP by 10%",
                    ...
                    context.TrainingService.RecordDecision(new ActionExplanation
                    {
                        ...
                        DetailedReason = $"Protraction on {targetName} at {capturedHpPercent:P0} HP. {(capturedTb ? "Applied before tank buster for max HP and healing received. " : "")}Protraction increases max HP by 10% and heals for the same amount. The 10s buff also increases healing received, making follow-up heals more effective. Free oGCD with no resource cost.",
                        Factors = factors,
